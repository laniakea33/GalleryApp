package com.dh.galleryapp.core.imagecache
...
class ImageCache @Inject constructor(
    ...
    fun requestImageSampling(url: String, width: Int, height: Int, id: String = "-1"): Job {
        ...
        return CoroutineScope(Dispatchers.IO).launch {
            Log.d("dhlog", "ImageCache requestSampledImage() : id$id $url, size : $width x $height, key : $key")
            ...
            if (isCachedInDisk(filePath)) {
                ...
                Log.d("dhlog", "ImageCache requestSampledImage() 디스크 캐시 있는 케이스, uiState 업데이트 함. key : $key")
    ...
    fun observe(downloadUrl: String, width: Int = -1, height: Int = -1): StateFlow<CacheResult> {
        val key = if (width > 0 && height > 0) "${URLEncoder.encode(downloadUrl)}_${width}_$height.jpg" else "${URLEncoder.encode(downloadUrl)}.jpg"
